...
1. Run the sql file to create the table and add 2 todos: `psql -U postgres -d todo_app_db < db/todo.sql`
    ...
    - This command will run the `todo.sql` inside the `todo_app_db` we just created. __Make sure__ you're in the root directory of the app when you run this command, otherwise the `db/todo.sql` will need to be altered.
...
1. Be sure to test this out in Postman with the provided collection of queries: `pru_todo_app.postman_collection.json`. When you open Postman, select "Open existing file", and open the collection.

  ![](./assets/postman-get-todos.png)

## CREATE A TODO

1. In `index.js`, you can add this code below the previous route. *Take a minute to consider what this code is doing.*
